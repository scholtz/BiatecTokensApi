...
namespace BiatecTokensApi.Services
{
    ...
    public class ComplianceOrchestrationService : IComplianceOrchestrationService
    {
            ...
            string correlationId)
        {
            ...
            AppendAuditEvent(decision, "CheckInitiated", ComplianceDecisionState.Pending, correlationId, null, null);

            _logger.LogInformation(
                "Initiating compliance check. DecisionId={DecisionId}, SubjectId={SubjectId}, CheckType={CheckType}, CorrelationId={CorrelationId}",
            ...
            string correlationId)
        {
            ...
            // If KYC is a hard failure (Rejected or Error), skip AML
            if (kycState == ComplianceDecisionState.Rejected || kycState == ComplianceDecisionState.Error)
            {
                decision.State = kycState;
                decision.ReasonCode = kycState == ComplianceDecisionState.Error ? "KYC_PROVIDER_ERROR" : "KYC_REJECTED";
            ...
            // Combine results: rejection takes priority
