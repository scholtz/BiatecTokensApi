...
namespace BiatecTokensTests
{
    ...
    public class ARC76MVPDeploymentPipelineE2EWorkflowTests
    {
        ...
        // ── Additional E2E workflow coverage ─────────────────────────────────────

        [Test]
        public async Task XA_TokenNameWithNumbers_FullLifecycle_Succeeds()
        {
            var svc = CreateService();
            var req = ValidRequest();
        ...
        [Test]
        public async Task XB_MultipleIdempotencyReplays_AllReturnSameId()
        {
            var svc = CreateService();
            var key = "replay-key-" + Guid.NewGuid();
            var req = ValidRequest(idempotencyKey: key);
            var r1 = await svc.InitiateAsync(req);
            var r2 = await svc.InitiateAsync(ValidRequest(idempotencyKey: key));
            var r3 = await svc.InitiateAsync(ValidRequest(idempotencyKey: key));
            Assert.That(r1.PipelineId, Is.EqualTo(r2.PipelineId));
        ...
        [Test]
        public async Task XC_CancelAtReadinessVerified_StatusIsCancelled()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XD_CancelAtValidationPending_AuditHasCancelEntry()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XE_Betanet_ASA_FullLifecycle_Succeeds()
        {
            var svc = CreateService();
            var req = new PipelineInitiateRequest
            {
        ...
        [Test]
        public async Task XF_CorrelationId_OnCancelResponse_IsNonEmpty()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var cancel = await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XG_StatusResponse_UpdatedAt_IsAfter_CreatedAt_AfterAdvance()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await Task.Delay(5);
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
            Assert.That(status!.UpdatedAt, Is.GreaterThanOrEqualTo(status.CreatedAt));
        }

        [Test]
        public async Task XH_CancelResponse_PreviousStage_IsCorrect()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XI_AdvanceResponse_NextAction_IsNonEmpty()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var adv = await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XJ_StatusResponse_RetryCount_IsZero_Initially()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task XK_StatusResponse_MaxRetries_MatchesRequest()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.MaxRetries = 8;
            var r = await svc.InitiateAsync(req);
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
            Assert.That(status!.MaxRetries, Is.EqualTo(8));
        }

        [Test]
        public async Task XL_GetStatus_WithCorrelationId_ReturnsThatCorrelationId()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var customCorr = "custom-status-corr-" + Guid.NewGuid();
            var status = await svc.GetStatusAsync(r.PipelineId!, customCorr);
            Assert.That(status!.CorrelationId, Is.EqualTo(customCorr));
        }

        [Test]
        public async Task XM_GetAudit_Returns_AllSucceeded_True_ForHappyPath()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
            Assert.That(audit.Events.All(e => e.Succeeded), Is.True,
                "All audit entries should have Succeeded=true for happy path");
        }

        [Test]
        public async Task XN_IdempotencyKey_ReplayAfterAdvance_ReturnsSamePipelineId()
        {
            var svc = CreateService();
            var key = "replay-after-adv-" + Guid.NewGuid();
            var req = ValidRequest(idempotencyKey: key);
            var r1 = await svc.InitiateAsync(req);
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r1.PipelineId });
            var r2 = await svc.InitiateAsync(ValidRequest(idempotencyKey: key));
        ...
        [Test]
        public async Task XO_InitiateAsync_Timestamp_IsRecentUtc()
        {
        ...
        [Test]
        public async Task XP_TwoPipelines_DifferentNetworks_BothComplete()
        {
            var svc = CreateService();
            var req1 = new PipelineInitiateRequest
            {
                TokenName = "TknTestnet", TokenStandard = "ASA", Network = "testnet",
                DeployerAddress = "ADDR_TN", MaxRetries = 3, CorrelationId = Guid.NewGuid().ToString()
            };
            var req2 = new PipelineInitiateRequest
            {
                TokenName = "TknBase", TokenStandard = "ERC20", Network = "base",
                DeployerAddress = "0xBASE_ADDR", MaxRetries = 3, CorrelationId = Guid.NewGuid().ToString()
            };
            var r1 = await svc.InitiateAsync(req1);
            var r2 = await svc.InitiateAsync(req2);
            await AdvanceAllStagesAsync(svc, r1.PipelineId!);
        ...
        [Test]
        public async Task XQ_AdvanceAsync_SchemaVersion_IsPresent()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var adv = await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XR_GetAuditAsync_Success_IsTrue()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task XS_CancelResponse_Success_IsTrue()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var cancel = await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XT_Aramidmain_ARC200_FullLifecycle_Succeeds()
        {
            var svc = CreateService();
            var req = new PipelineInitiateRequest
            {
                TokenName = "AramidToken", TokenStandard = "ARC200", Network = "aramidmain",
                DeployerAddress = "ARAMID_ADDR", MaxRetries = 3, CorrelationId = Guid.NewGuid().ToString()
            };
            var r = await svc.InitiateAsync(req);
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task XU_AuditEvents_EachHaveNonNullPipelineId()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task XV_InitiateAsync_ReadinessStatus_NotChecked()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task XW_AdvanceFromDeploymentQueued_IsDeploymentActive()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 6; i++)
        ...
        [Test]
        public async Task XX_AdvanceFromDeploymentActive_IsDeploymentConfirmed()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 7; i++)
        ...
        [Test]
        public async Task XY_AdvanceFromDeploymentConfirmed_IsCompleted()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 8; i++)
        ...
        // ── New E2E workflow tests ────────────────────────────────────────────────

        [Test]
        public async Task E2E_PipelineId_IsGuidFormat()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
        ...
        [Test]
        public async Task E2E_AdvanceToReadinessVerified_PreviousStageIsPendingReadiness()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var adv = await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_CancelAfterThreeAdvances_ReturnsSuccess()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 3; i++)
        ...
        [Test]
        public async Task E2E_StatusAfterCancel_StageIsCancelled()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_AdvanceAfterCancel_ReturnsTerminalError()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_IdempotentReplay_SamePipelineId_ThirdRun()
        {
            var svc = CreateService();
            var key = Guid.NewGuid().ToString();
            var r1 = await svc.InitiateAsync(ValidRequest(idempotencyKey: key));
            await svc.InitiateAsync(ValidRequest(idempotencyKey: key));
            var r3 = await svc.InitiateAsync(ValidRequest(idempotencyKey: key));
            Assert.That(r3.PipelineId, Is.EqualTo(r1.PipelineId));
            Assert.That(r3.IsIdempotentReplay, Is.True);
        }
        ...
        [Test]
        public async Task E2E_AuditEvents_AllHaveNonEmptyEventId()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
        ...
        [Test]
        public async Task E2E_AuditEvents_AllHaveMatchingPipelineId()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
        ...
        [Test]
        public async Task E2E_RetryAsync_OnNonFailedPipeline_ReturnsNotInFailedState()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
        ...
        [Test]
        public async Task E2E_IdempotencyConflict_DifferentNetwork_ReturnsConflict()
        {
            var svc = CreateService();
            var key = Guid.NewGuid().ToString();
            var r1 = ValidRequest(idempotencyKey: key);
            r1.Network = "testnet";
            await svc.InitiateAsync(r1);
            var r2 = ValidRequest(idempotencyKey: key);
            r2.Network = "mainnet";
            var result = await svc.InitiateAsync(r2);
            Assert.That(result.ErrorCode, Is.EqualTo("IDEMPOTENCY_KEY_CONFLICT"));
        }

        [Test]
        public async Task E2E_StatusResponse_ReadinessStatus_NotChecked_Initially()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task E2E_StatusResponse_ReadinessStatus_Ready_AfterFirstAdvance()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_EmptyDeployerAddress_ReturnsValidationError()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.DeployerAddress = "";
        ...
        [Test]
        public async Task E2E_EmptyTokenName_ReturnsValidationError()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenName = "";
        ...
        [Test]
        public async Task E2E_EmptyNetwork_ReturnsValidationError()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.Network = "";
        ...
        [Test]
        public async Task E2E_FivePipelines_AllHaveUniqueIds()
        {
            var svc = CreateService();
            var ids = new HashSet<string>();
            for (int i = 0; i < 5; i++)
        ...
        [Test]
        public async Task E2E_CorrelationId_PropagatesIntoAuditEvents()
        {
            var svc = CreateService();
            var corrId = "e2e-corr-propagation-" + Guid.NewGuid();
            var r = await svc.InitiateAsync(ValidRequest(correlationId: corrId));
            var audit = await svc.GetAuditAsync(r.PipelineId!, corrId);
            Assert.That(audit.Events, Is.Not.Empty);
            foreach (var ev in audit.Events!)
        ...
        [Test]
        public async Task E2E_MaxRetries_Five_IsAccepted()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.MaxRetries = 5;
        ...
        [Test]
        public async Task E2E_ARC3Standard_OnTestnet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC3";
        ...
        [Test]
        public async Task E2E_ASAStandard_OnMainnet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ASA";
        ...
        [Test]
        public async Task E2E_ARC1400Standard_OnVoimain_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC1400";
        ...
        [Test]
        public async Task E2E_ERC20Standard_OnBase_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ERC20";
        ...
        [Test]
        public async Task E2E_ARC200Standard_OnBetanet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC200";
        ...
        [Test]
        public async Task E2E_ARC200Standard_OnAramidmain_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC200";
        ...
        [Test]
        public async Task E2E_AuditEventIds_AreUniqueAcrossFullLifecycle()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
        ...
        [Test]
        public async Task E2E_StatusPipelineId_MatchesInitiateResponse()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task E2E_AuditPipelineId_MatchesInitiateResponse()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task E2E_CancelAtReadinessVerified_AuditGrows()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_InitiateAsync_SuccessTrue_And_PipelineIdPresent()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            Assert.That(r.Success, Is.True);
        ...
        [Test]
        public async Task E2E_AdvanceAsync_NullPipelineId_ReturnsMissingError()
        {
            var svc = CreateService();
        ...
        [Test]
        public async Task E2E_RetryAsync_FromInitiatedPipeline_ReturnsNotInFailedState()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var retry = await svc.RetryAsync(new PipelineRetryRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_CancelAtCompliancePassed_Succeeds()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 4; i++)
        ...
        [Test]
        public async Task E2E_Idempotency_NullKey_CreatesTwoPipelines_V2()
        {
            var svc = CreateService();
            var r1 = await svc.InitiateAsync(ValidRequest(idempotencyKey: null));
            var r2 = await svc.InitiateAsync(ValidRequest(idempotencyKey: null));
        ...
        [Test]
        public async Task E2E_AuditEvents_AllHaveNonEmptyEventId_FullLifecycle()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
        ...
        [Test]
        public async Task E2E_SchemaVersion_IsOneZeroZero_InAuditResponse()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
        ...
        [Test]
        public async Task E2E_ThreeConcurrentFullLifecycles_AllReachCompleted()
        {
        ...
        [Test]
        public async Task E2E_InitiateAsync_FailureCategory_IsNone_OnValidRequest()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
        ...
        [Test]
        public async Task E2E_GetStatus_AfterAdvanceAndCancel_IsIsolatedPerPipeline()
        {
            var svc = CreateService();
            var r1 = await svc.InitiateAsync(ValidRequest());
            var r2 = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r1.PipelineId });
            await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r2.PipelineId });
            var s1 = await svc.GetStatusAsync(r1.PipelineId!, null);
            var s2 = await svc.GetStatusAsync(r2.PipelineId!, null);
            Assert.That(s1!.Stage, Is.EqualTo(PipelineStage.ReadinessVerified));
            Assert.That(s2!.Stage, Is.EqualTo(PipelineStage.Cancelled));
        }

        [Test]
        public async Task E2E_InitiateAsync_WithMaxRetries_Zero_Succeeds()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.MaxRetries = 0;
        ...
        [Test]
        public async Task E2E_CancelAsync_NullPipelineId_ReturnsError()
        {
            var svc = CreateService();
        ...
        [Test]
        public async Task E2E_ARC1400_OnMainnet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC1400";
        ...
        [Test]
        public async Task E2E_SchemaVersion_Consistent_AcrossInitiateAndAudit()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
            Assert.That(r.SchemaVersion, Is.EqualTo(audit.SchemaVersion));
        }

        [Test]
        public async Task E2E_AdvanceFromReadinessVerified_StageIsValidationPending()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_AuditEvents_AllHavePipelineIdSet()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await AdvanceAllStagesAsync(svc, r.PipelineId!);
        ...
        [Test]
        public async Task E2E_GetStatus_Null_UnknownId_ReturnsNull()
        {
            var svc = CreateService();
            var status = await svc.GetStatusAsync("unknown-pipeline-xyz", null);
            Assert.That(status, Is.Null);
        }

        [Test]
        public async Task E2E_ASA_OnMainnet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ASA";
        ...
        [Test]
        public async Task E2E_ARC3_OnVoimain_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC3";
        ...
        [Test]
        public async Task E2E_ERC20_OnBase_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ERC20";
        ...
        [Test]
        public async Task E2E_ARC200_OnBetanet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC200";
        ...
        [Test]
        public async Task E2E_CancelAt_CompliancePending_AuditHasCancelEvent()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 4; i++)
        ...
        [Test]
        public async Task E2E_AuditTrail_FirstEvent_IsInitiate()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
            Assert.That(audit.Events!.First().Operation, Is.EqualTo("Initiate"));
        }

        [Test]
        public async Task E2E_AuditTrail_SecondEvent_IsAdvance_AfterFirstAdvance()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = r.PipelineId });
            var audit = await svc.GetAuditAsync(r.PipelineId!, null);
            Assert.That(audit.Events![1].Operation, Is.EqualTo("Advance"));
        }

        [Test]
        public async Task E2E_Idempotency_ThirdReplay_ReturnsSamePipelineId()
        {
            var svc = CreateService();
            var key = "e2e-idempotency-" + Guid.NewGuid();
            var req = ValidRequest(idempotencyKey: key);
            var r1 = await svc.InitiateAsync(req);
            var r2 = await svc.InitiateAsync(req);
            var r3 = await svc.InitiateAsync(req);
        ...
        [Test]
        public async Task E2E_StatusAfterCancel_IsNotActive()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r.PipelineId });
        ...
        [Test]
        public async Task E2E_RetryAsync_UnknownId_ReturnsError()
        {
            var svc = CreateService();
            var retry = await svc.RetryAsync(new PipelineRetryRequest { PipelineId = "no-such-id" });
            Assert.That(retry.Success, Is.False);
        }

        [Test]
        public async Task E2E_AdvanceAsync_UnknownId_ReturnsError()
        {
            var svc = CreateService();
            var adv = await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = "no-such-id" });
            Assert.That(adv.Success, Is.False);
        }

        [Test]
        public async Task E2E_CancelAsync_UnknownId_ReturnsError()
        {
            var svc = CreateService();
            var cancel = await svc.CancelAsync(new PipelineCancelRequest { PipelineId = "no-such-id" });
            Assert.That(cancel.Success, Is.False);
        }

        [Test]
        public async Task E2E_TwoPipelines_CancelOne_OtherIsUnaffected()
        {
            var svc = CreateService();
            var r1 = await svc.InitiateAsync(ValidRequest());
            var r2 = await svc.InitiateAsync(ValidRequest());
            await svc.CancelAsync(new PipelineCancelRequest { PipelineId = r1.PipelineId });
            var s2 = await svc.GetStatusAsync(r2.PipelineId!, null);
            Assert.That(s2!.Stage, Is.EqualTo(PipelineStage.PendingReadiness));
        }

        [Test]
        public async Task E2E_Status_TokenName_MatchesRequestedTokenName()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenName = "SpecificToken";
            var r = await svc.InitiateAsync(req);
            var status = await svc.GetStatusAsync(r.PipelineId!, null);
            Assert.That(status!.TokenName, Is.EqualTo("SpecificToken"));
        }

        [Test]
        public async Task E2E_Status_TokenStandard_MatchesRequest()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC3";
        ...
        [Test]
        public async Task E2E_AdvanceThreeTimesFromPending_AuditHasFourEvents()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            for (int i = 0; i < 3; i++)
        ...
        [Test]
        public async Task E2E_FailureCategory_IsNone_ForSuccessfulInitiate()
        {
            var svc = CreateService();
            var r = await svc.InitiateAsync(ValidRequest());
            Assert.That(r.FailureCategory, Is.EqualTo(FailureCategory.None));
        ...
        [Test]
        public async Task E2E_ARC200_OnMainnet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC200";
        ...
        [Test]
        public async Task E2E_ASA_OnVoimain_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ASA";
        ...
        [Test]
        public async Task E2E_ARC3_OnAramidmain_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC3";
        ...
        [Test]
        public async Task E2E_ARC1400_OnBetanet_FullCycle_Completes()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.TokenStandard = "ARC1400";
        ...
        [Test]
        public async Task E2E_Status_Network_MatchesRequest()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.Network = "mainnet";
        ...
        [Test]
        public async Task E2E_AdvanceAsync_NullRequest_ReturnsError()
        {
            var svc = CreateService();
            var adv = await svc.AdvanceAsync(new PipelineAdvanceRequest { PipelineId = null });
            Assert.That(adv.Success, Is.False);
        ...
        [Test]
        public async Task E2E_GetAuditAsync_UnknownId_ReturnsEmptyEvents()
        {
            var svc = CreateService();
            var audit = await svc.GetAuditAsync("no-pipeline-here", null);
            Assert.That(audit.Events, Is.Empty);
        }

        [Test]
        public async Task E2E_RetryAsync_NullId_HasErrorCode()
        {
            var svc = CreateService();
            var retry = await svc.RetryAsync(new PipelineRetryRequest { PipelineId = null });
        ...
        [Test]
        public async Task E2E_InitiateAsync_MaxRetries_Five_Succeeds()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.MaxRetries = 5;
        ...
        [Test]
        public async Task E2E_Status_MaxRetries_MatchesRequest()
        {
            var svc = CreateService();
            var req = ValidRequest();
            req.MaxRetries = 7;
