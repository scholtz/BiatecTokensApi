...
namespace BiatecTokensTests
{
    ...
    public class HealthCheckIntegrationTests
    {
        ...
        [Test]
        public async Task SwaggerJsonEndpoint_ReturnsOpenApiDocument()
        {
            // Act
            ...
            // Assert
            Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK),
                $"Swagger JSON endpoint should return 200 OK. Response body: {content}");
            Assert.That(content, Is.Not.Null.And.Not.Empty, "Swagger JSON should return content");

            using var document = JsonDocument.Parse(content);
            Assert.That(document.RootElement.TryGetProperty("openapi", out var openApiVersion), Is.True,
                "Swagger JSON should include the OpenAPI version field");
            Assert.That(openApiVersion.GetString(), Is.Not.Null.And.Not.Empty,
                "OpenAPI version should not be empty");
            Assert.That(document.RootElement.TryGetProperty("paths", out _), Is.True,
                "Swagger JSON should include the paths section");
        }
